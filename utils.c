...
struct Node * addBrotherNode(struct Node * n1, struct Node * n2){
    struct Node * t = n1;
    if(t != 0){
        t = createSyntaxTreeNode(t->nodeType, t->attr.ch, t->childrenNode[0], t->childrenNode[1], t->childrenNode[2]);
        t->brotherNode = n2;
    } else
        t = n2;
    return t;
...
void fun_noType(struct Node * tree){ /*empty*/ }
